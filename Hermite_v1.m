function [] = Hermite_v1(a,b,resolution,matrice)
    ...
    % recuperation de la valeur des alpha_i
    prompt = {'Enter alpha value:'};
    dlgtitle = 'Input';
    dims = [1 50];
    definput = {'alpha = '};
    alpha=inputdlg(prompt,dlgtitle,dims,definput);
    alpha=str2double(alpha);
    if P==2 % m0 et mN automatique
        derives(1,1)=(1/alpha)*(matrice(1,2)-matrice(1,1));
        derives(2,1)=(1/alpha)*(matrice(2,2)-matrice(2,1));
        derives(1,N)=(1/alpha)*(matrice(1,N)-matrice(1,N-1));
        derives(2,N)=(1/alpha)*(matrice(2,N)-matrice(2,N-1));
    end
    % calcul des mi
    miderive = derive_v1(alpha,matrice);
    ...
    str = strcat('Spline Hermite-v1 alpha =  ',num2str(alpha));
    plot(courbe_interpole(1,:),courbe_interpole(2,:),'DisplayName',str);
    legend
    if L==1
        courbure(a,b,resolution,derives,matrice);
        str = strcat('Courbure Hermite-v1 alpha =  ',num2str(alpha));
        legend(str)
    end